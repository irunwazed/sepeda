				...
				<form action="" id="form-data">
					...
					<div class="row">
						...
						<div class="form-group col-sm-12">
							...
							<fieldset class="form-group">
								<textarea name="renstra_sasaran_indikator_nama" class="form-control" cols="30" rows="5" readonly></textarea>
							</fieldset>
						</div>
						<div class="form-group col-sm-4">
							<label for="basicInput">Target Kinerja</label>
							<fieldset class="form-group">
								<input type="text" name="renstra_sasaran_indikator_target" class="form-control" readonly>
							</fieldset>
						</div>
						<div class="form-group col-sm-4">
							<label for="basicInput">Satuan</label>
							<fieldset class="form-group">
								<input type="text" name="renstra_sasaran_indikator_satuan" class="form-control" readonly>
							</fieldset>
						</div>
...
$('li[data-menu-bar="realisasi-renstra"]').addClass("active pcoded-trigger");
	...
	function setCreate() {
		...
		$('textarea[name="rpjmd_sasaran_nama"]').html('');
	...
	function setUpdate(id) {
		...
		$.when(sendAjax(url, {}, 'get', '#form-data')).done(function(res) {
			if (res.status) {
				dataPilih = res.data;
				$('input[name="id"]').val(res.data.renstra_sasaran_kode);
				$('input[name="kode"]').val(res.data.renstra_tujuan_kode
				+"-"+res.data.renstra_sasaran_kode
				+"-"+res.data.renstra_sasaran_indikator_kode);

				$('input[name="realisasi_kinerja"]').val(dataPilih['renstra_sasaran_indikator_th' + tahun_ke +
					'_realisasi_target']);
				$('input[name="realisasi_pagu"]').val(dataPilih['renstra_sasaran_indikator_th' + tahun_ke + '_realisasi_pagu']);

				$('textarea[name="renstra_sasaran_nama"]').html(res.data.renstra_sasaran_nama);
				...
				$('input[name="renstra_sasaran_indikator_target"]').val(res.data['renstra_sasaran_indikator_th' + tahun_ke +
					'_target']);


				let arrJson = JSON.parse(res.data.renstra_sasaran_indikator_nilai_json);
				let strJson = '';

	...
	public function cetak(Request $request){
		...
		foreach($data as $row){
			...
			if($program_id != $row->id){
				...
				foreach($dataAll[$index]['data'] as $rowProg){
					for($idxTahun = 1; $idxTahun <= 5; $idxTahun++){
						...
						->where('ref_rkpd_sub_kegiatan.permen_ver', $rowProg->permen_ver)
						->where('ref_rkpd_sub_kegiatan.urusan_kode', $rowProg->urusan_kode)
						->where('ref_rkpd_sub_kegiatan.bidang_kode', $rowProg->bidang_kode)
						->where('ref_rkpd_sub_kegiatan.program_kode', $rowProg->program_kode)
						->where('ref_rkpd_sub_kegiatan.opd_id', $rowProg->opd_id)
						->get();
						...
						foreach($dataAll[$index]['sub_kegiatan'][$idxTahun]['data'] as $rowTahunan){
							...
							$sub_pagu_realisasi = @$rowTahunan->rkpd_sub_kegiatan_indikator_pagu;
							...
							$dataAll[$index]['dataPagu'][$idxTahun]['pagu_rasio'] = $dataAll[$index]['dataPagu'][$idxTahun]['pagu']==0?0:$dataAll[$index]['dataPagu'][$idxTahun]['realisasi_pagu']/$dataAll[$index]['dataPagu'][$idxTahun]['pagu'];
							...
							$dataAll[$sasaran_index]['dataPagu'][$idxTahun]['pagu_rasio'] = $dataAll[$sasaran_index]['dataPagu'][$idxTahun]['pagu']==0?0:$dataAll[$sasaran_index]['dataPagu'][$idxTahun]['realisasi_pagu']/$dataAll[$sasaran_index]['dataPagu'][$idxTahun]['pagu'];
							...
							$dataAll[$tujuan_index]['dataPagu'][$idxTahun]['pagu_rasio'] = $dataAll[$tujuan_index]['dataPagu'][$idxTahun]['pagu']==0?0:$dataAll[$tujuan_index]['dataPagu'][$idxTahun]['realisasi_pagu']/$dataAll[$tujuan_index]['dataPagu'][$idxTahun]['pagu'];
						}
					}

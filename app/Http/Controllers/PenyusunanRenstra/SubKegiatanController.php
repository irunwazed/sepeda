...
class SubKegiatanController extends Controller
{
	...
	{
		$this->table = 'ta_renstra_sub_kegiatan_indikator';
	}
	...
	public function view($kode)
	{
		...
		// echo "<pre>";
		// print_r($data);
		$dataAwal = DB::table('ref_renstra_kegiatan')
			...
			->where('ref_renstra_kegiatan.id', $kode)
			->first();

			// print_r($dataAwal);
			// die();

		$dataSubKegiatan = DB::table('ref_sub_kegiatan')
			->where('ref_sub_kegiatan.permen_ver', @$dataAwal->permen_ver)
			->where('ref_sub_kegiatan.urusan_kode', @$dataAwal->urusan_kode)
			->where('ref_sub_kegiatan.bidang_kode', @$dataAwal->bidang_kode)
			->where('ref_sub_kegiatan.program_kode', @$dataAwal->program_kode)
			->where('ref_sub_kegiatan.kegiatan_kode', @$dataAwal->kegiatan_kode)
			->get();
		$kirim = [
			'dataAwal' => $dataAwal,
		...
		return view('components/penyusunan-renstra/sub-kegiatan', $kirim);
	}
	...
	public function getQuery($request, $kode)
	{
		$data = DB::table($this->table)
			->select(
				$this->table . '.*',
				'ref_renstra_sub_kegiatan.permen_ver',
				'ref_renstra_sub_kegiatan.urusan_kode',
				'ref_renstra_sub_kegiatan.bidang_kode',
				'ref_renstra_sub_kegiatan.program_kode',
				'ref_renstra_sub_kegiatan.kegiatan_kode',
				'ref_renstra_sub_kegiatan.sub_kegiatan_kode',
				'program_nama',
				'kegiatan_nama',
				'sub_kegiatan_nama'
			)
			...
			->where('ref_renstra_sub_kegiatan.renstra_kegiatan_id', $kode);
	...
	public function getData(Request $request, $kode = null, $id = null)
	{
		...
		return DataTables::of($data->get())
			->addColumn('kode', function ($row) {
				return (@$row->urusan_kode == 0 ? 'X' : $row->urusan_kode) . "." . $this->setKode(@$row->bidang_kode) . '.' . $this->setKode(@$row->program_kode) . '.' . $this->setKode(@$row->kegiatan_kode, 2) . '.' . $this->setKode(@$row->sub_kegiatan_kode, 3);
				// return '';
			})
			...
			->addColumn('th0_target', function ($row) {
				$nilai = $row->renstra_sub_kegiatan_indikator_th0_realisasi_target;
				$hasil = $nilai;
			...
			->addColumn('th1_target', function ($row) {
				$nilai = $row->renstra_sub_kegiatan_indikator_th1_target;
				$hasil = $nilai;
				if (@$row->renstra_sub_kegiatan_indikator_nilai_jenis == 2) {
					$arr = json_decode($row->renstra_sub_kegiatan_indikator_nilai_json, true);
					for ($i = 0; $i < count($arr); $i++) {
			...
			->addColumn('th2_target', function ($row) {
				$nilai = $row->renstra_sub_kegiatan_indikator_th2_target;
				$hasil = $nilai;
				if (@$row->renstra_sub_kegiatan_indikator_nilai_jenis == 2) {
					$arr = json_decode($row->renstra_sub_kegiatan_indikator_nilai_json, true);
					for ($i = 0; $i < count($arr); $i++) {
			...
			->addColumn('th3_target', function ($row) {
				$nilai = $row->renstra_sub_kegiatan_indikator_th3_target;
				$hasil = $nilai;
				if (@$row->renstra_sub_kegiatan_indikator_nilai_jenis == 2) {
					$arr = json_decode($row->renstra_sub_kegiatan_indikator_nilai_json, true);
					for ($i = 0; $i < count($arr); $i++) {
			...
			->addColumn('th4_target', function ($row) {
				$nilai = $row->renstra_sub_kegiatan_indikator_th4_target;
				$hasil = $nilai;
				if (@$row->renstra_sub_kegiatan_indikator_nilai_jenis == 2) {
					$arr = json_decode($row->renstra_sub_kegiatan_indikator_nilai_json, true);
					for ($i = 0; $i < count($arr); $i++) {
			...
			->addColumn('th5_target', function ($row) {
				$nilai = $row->renstra_sub_kegiatan_indikator_th5_target;
				$hasil = $nilai;
				if (@$row->renstra_sub_kegiatan_indikator_nilai_jenis == 2) {
					$arr = json_decode($row->renstra_sub_kegiatan_indikator_nilai_json, true);
					for ($i = 0; $i < count($arr); $i++) {
	...
	public function action($request, $action = 'create', $kode = '')
	{
		...
		if ($validator->fails()) {
			...
			$cekKegiatan = DB::table('ref_renstra_sub_kegiatan')
				->where($data)->first();
			if (!@$cekKegiatan->id) {
				$kegiatanId = DB::table('ref_renstra_sub_kegiatan')->insertGetId($data);
			} else {
				$kegiatanId = $cekKegiatan->id;
			}

			$data = [
				'renstra_sub_kegiatan_indikator_nama' => $request->renstra_sub_kegiatan_indikator_nama,
				'renstra_sub_kegiatan_indikator_satuan' => $request->renstra_sub_kegiatan_indikator_satuan,
				'renstra_sub_kegiatan_indikator_th0_realisasi_target' => $request->renstra_sub_kegiatan_indikator_th0_realisasi_target,
				'renstra_sub_kegiatan_indikator_th1_target' => $request->renstra_sub_kegiatan_indikator_th1_target,
				'renstra_sub_kegiatan_indikator_th2_target' => $request->renstra_sub_kegiatan_indikator_th2_target,
				'renstra_sub_kegiatan_indikator_th3_target' => $request->renstra_sub_kegiatan_indikator_th3_target,
				'renstra_sub_kegiatan_indikator_th4_target' => $request->renstra_sub_kegiatan_indikator_th4_target,
				'renstra_sub_kegiatan_indikator_th5_target' => $request->renstra_sub_kegiatan_indikator_th5_target,

				'renstra_sub_kegiatan_indikator_th1_pagu' => $request->renstra_sub_kegiatan_indikator_th1_pagu,
				'renstra_sub_kegiatan_indikator_th2_pagu' => $request->renstra_sub_kegiatan_indikator_th2_pagu,
				'renstra_sub_kegiatan_indikator_th3_pagu' => $request->renstra_sub_kegiatan_indikator_th3_pagu,
				'renstra_sub_kegiatan_indikator_th4_pagu' => $request->renstra_sub_kegiatan_indikator_th4_pagu,
				'renstra_sub_kegiatan_indikator_th5_pagu' => $request->renstra_sub_kegiatan_indikator_th5_pagu,
				'updated_at' => $date,
			];

			if ($action == 'create') {
				$json = [];
				if (@$request->renstra_sub_kegiatan_indikator_nilai_jenis == 2) {
					$idx = 0;
				...
				$jenis = @$request->renstra_sub_kegiatan_indikator_nilai_jenis ? $request->renstra_sub_kegiatan_indikator_nilai_jenis : 1;
				$data['renstra_sub_kegiatan_indikator_nilai_jenis'] = $jenis;
				$data['renstra_sub_kegiatan_indikator_nilai_json'] = $json;
				$data['renstra_sub_kegiatan_id'] = $kegiatanId;

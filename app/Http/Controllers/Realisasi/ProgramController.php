...
class ProgramController extends Controller
{
	...
	public function view()
	{
		...
		return view('components/realisasi/program', $kirim);
	}
	...
	public function getQuery($request)
	{
		$data = DB::table($this->table)
			->select(
				$this->table . '.*',
				'ref_rpjmd_program.permen_ver',
				'ref_rpjmd_program.urusan_kode',
				'ref_rpjmd_program.bidang_kode',
				'ref_rpjmd_program.program_kode',
				'program_nama'
			)
			...
			->orderBy('ref_rpjmd_program.permen_ver', 'asc')
			->orderBy('ref_rpjmd_program.urusan_kode', 'asc')
			->orderBy('ref_rpjmd_program.bidang_kode', 'asc')
			->orderBy('ref_rpjmd_program.program_kode', 'asc');
    ...
    $tahun_ke = session('tahun');
		return DataTables::of($data->get())
			->addColumn('target', function ($row) use ($tahun_ke) {
				$th = 'rpjmd_program_indikator_th' . $tahun_ke . '_target';
				$nilai = $row->$th;
				$hasil = $nilai;
			...
			->addColumn('realisasi_target', function ($row) use ($tahun_ke) {
				$th = 'rpjmd_program_indikator_th' . $tahun_ke . '_realisasi_target';
				$nilai = $row->$th;
				$hasil = $nilai;
				if (@$row->rpjmd_program_indikator_nilai_jenis == 2) {
					$arr = json_decode($row->rpjmd_program_indikator_nilai_json, true);
	...
	public function action($request, $action = 'create', $kode = '')
	{
		...
		if ($validator->fails()) {
			...
			$data = [
				'rpjmd_program_indikator_th' . session('tahun') . '_realisasi_target' => $request->realisasi_kinerja,
				'updated_at' => $date,
			];

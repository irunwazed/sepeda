...
class SasaranController extends Controller
{
  ...
  public function view(){
    ...
    return view('components/realisasi/sasaran', $kirim);
  }
  ...
  public function getData(Request $request, $id = null){
    ...
    return DataTables::of($data->get())
		->addColumn('target', function ($row) use ($tahun_ke) {
			$th = 'rpjmd_sasaran_indikator_th'.$tahun_ke.'_target';
			$nilai = $row->$th;
			$hasil = $nilai;
		...
		->addColumn('realisasi_target', function ($row) use ($tahun_ke) {
			$th = 'rpjmd_sasaran_indikator_th'.$tahun_ke.'_realisasi_target';
			$nilai = $row->$th;
			$hasil = $nilai;
			if (@$row->rpjmd_sasaran_indikator_nilai_jenis == 2) {
				$arr = json_decode($row->rpjmd_sasaran_indikator_nilai_json, true);
		...
		->addColumn('pagu', function($row){
			...
			// ->where('ref_rpjmd_sasaran.id', $row->rpjmd_sasaran_id)
  ...
  public function action($request, $action = 'create', $kode = ''){
    ...
    if ($validator->fails()) {
      $error = $validator->errors()->all();
			...
			$data = [
				'rpjmd_sasaran_indikator_th'.session('tahun').'_realisasi_target' => $request->realisasi_kinerja,
				'updated_at' => $date,
			];
